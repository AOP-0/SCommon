 ...
 */
@IBDesignable
open class P_AM_SkyFloatingLabelTextField: UITextField { // swiftlint:disable:this type_body_length
    /**
    ...
    @IBInspectable dynamic open var placeholderColor1: UIColor = UIColor.lightGray {
        didSet {
            updatePlaceholder()
        }
    }
    ...
    fileprivate func updatePlaceholder() {
        ...
        let color = isEnabled ? placeholderColor1 : disabledColor
        #if swift(>=4.2)
    ...
    @IBInspectable dynamic open var errLineHeight: CGFloat = 1.5 {
        didSet {
            updateLineView()
            setNeedsDisplay()
        }
    ...
    @IBInspectable dynamic open var textLeftMagin: CGFloat = 0
    
    @IBInspectable dynamic open var textRightMagin: CGFloat = 0
    
    @IBInspectable dynamic open var errTextMagin: CGFloat = 0
    
    @IBInspectable dynamic open var lineTextMagin: CGFloat = 5
    
    fileprivate var lineFactHeight: CGFloat {
        
        if hasErrorMessage {
            return errLineHeight
    ...
    */
    override public init(frame: CGRect) {
        ...
        init_P_AM_SkyFloatingLabelTextField()
    }
    ...
    fileprivate final func init_P_AM_SkyFloatingLabelTextField() {
        borderStyle = .none
        createTitleLabel()
        createLineView()
        createErrorLabel()
    ...
    fileprivate func addEditingChangedObserver() {
        self.addTarget(self, action: #selector(P_AM_SkyFloatingLabelTextField.editingChanged), for: .editingChanged)
    }
...
//        lineHeight = 2.0 * onePixel
    ...
    */
    override open func textRect(forBounds bounds: CGRect) -> CGRect {
        ...
        let height = superRect.size.height - titleHeight - lineHeight - lineTextMagin - errorMagrinHeight()
      
        let rect = CGRect(
            x: superRect.origin.x + textLeftMagin,
            y: titleHeight,
            width: superRect.size.width - textLeftMagin - textRightMagin,
            height: height
        )
        return rect
    }
    ...
    override open func placeholderRect(forBounds bounds: CGRect) -> CGRect {
        let height = bounds.size.height - titleHeight() - lineFactHeight - errorMagrinHeight()
    
        let rect = CGRect(
            x: 0 + textLeftMagin,
            y: titleHeight(),
            width: bounds.size.width - textLeftMagin - textRightMagin,
            height: height
        )
        return rect
    }
    ...
    */
    open func titleLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if editing {
            return CGRect(x: textLeftMagin, y: 0, width: bounds.size.width - textLeftMagin - textRightMagin, height: titleHeight())
        }
        return CGRect(x: textLeftMagin, y: titleHeight(), width: bounds.size.width - textLeftMagin - textRightMagin, height: titleHeight())
    }
    ...
    open func errorLabelRectForBounds(_ bounds: CGRect, editing: Bool) -> CGRect {
        if errorMessagePlacement == .default {
            ...
            if editing {
                let originY =  lineRect.maxY 
                return CGRect(x: 0 + textLeftMagin, y: originY + errTextMagin, width: bounds.size.width - textLeftMagin - textRightMagin, height: errorHeight())
            }
            let originY = lineRect.origin.y + lineFactHeight + errTextMagin
            return CGRect(x: 0 + textLeftMagin, y: originY, width: bounds.size.width - textLeftMagin - textRightMagin, height: errorHeight())
        }
    }
    ...
    open func lineViewRectForBounds(_ bounds: CGRect) -> CGRect {
        let height = lineFactHeight
        if errorMessagePlacement == .bottom {
            return CGRect(x: 0, y: textRect(forBounds: bounds).maxY + lineTextMagin, width: bounds.size.width, height: height)
        } else {
    ...
    open func errorHeight() -> CGFloat {
        ...
        errorMessagePlacement == .bottom ? errorLabel.font.lineHeight : 0
        
       
    }
    
    open func errorMagrinHeight() -> CGFloat {
        
        errorMessagePlacement == .bottom ? errorLabel.font.lineHeight + errTextMagin : 0
    ...
    override open var intrinsicContentSize: CGSize {
        CGSize(width: bounds.size.width, height: titleHeight() + textHeight() + lineTextMagin + lineHeight  +  errorMagrinHeight())
    }
    ...
    fileprivate func selectedTitleOrTitlePlaceholder() -> String? {
        guard hasTitle else {
            ...
            return titleFormatter(selectedTitle ?? title ?? "")
        }
